...
const itemsList = pictures.map(({ width, height, alt, url }) => {
  return `<li>
  <a href="#">
...
galeeryRef.insertAdjacentHTML('beforeend', itemsList.join(''));
// galeeryRef.innerHTML = itemsList.join('');
// galeeryRef.insertAdjacentHTML('beforeend', itemsList.join(''));

console.log(galeeryRef.children);
const liRefs = [...galeeryRef.children];
liRefs.forEach(item => {
  item.style.flexBasis = '90%';
});
